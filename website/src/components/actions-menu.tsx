export function ActionsMenu({
  ...
  setCurrentMode,
	...
	return (
      ...
      <button onClick={() => setCurrentMode('move')} class="rounded-full h-4 w-4 bg-gray-500"></button>
			<button onClick={() => setCurrentMode('draw-green')} class="rounded-full h-4 w-4 bg-emerald-500"></button>
			<button onClick={() => setCurrentMode('draw-red')} class="rounded-full h-4 w-4 bg-red-500"></button>
		</div>
	);
}
